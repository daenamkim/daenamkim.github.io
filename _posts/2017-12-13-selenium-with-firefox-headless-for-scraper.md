...
- selenium
- python
twitter_text: 'Selenium과 Firefox Headless로 Web Scraper 만들기.'
introduction: 'Selenium과 Firefox Headless로 Web Scraper 만들기.'
---
...
현재 서비스 중인 [ListeningMind SEO 기능](http://www.listeningmind.com/ko/seo-check/)에서 이미 Chrome Driver를 사용하고 있었으나 원인 불명으로 계속해서 오류가 발생했다. 말 그대로 원인 불명이다. ;;
...
Firefox + Gecko Driver를 사용하면 몇 가지 작업을 더 해주어야 하는데 모두 정리하면 다음과 같다.
- Gecko Driver 설치.
- Firefox 설치.
...
## Gecko Driver 설치
[Mozila](https://www.mozilla.org/ko/)에서 GitHub에 공개해 놓은 파일을 내려 받아서 이동 시키기만 하면 된다.
{% highlight shell %}
...
Xvfb를 백그라운드로 실행하고 `DISPLAY` 환경 변수도 지정 해준다. `:10`은 Xvfb가 사용할 화면 번호인데 적당히 지정 해주면 된다.
{% highlight shell %}
$ Xvfb :10 -ac &
$ export DISPLAY=:10
{% endhighlight %}
...
시간이 된다면 정신 건강을 위해서라도 꼭 사용하길 권장한다. 설치 방법은 위 링크를 참조하길 바란다.

## Selenium 동작 확인
이제 Selenium을 사용할 준비가 되었으니 동작을 확인할 테스트 코드를 작성하자.
{% highlight python %}
# -*- coding: utf_8 -*-
...
driver.get('http://www.ascentnet.co.jp')
...
테스트 코드도 작성 하였으니 다음과 같이 실행한다.
{% highlight shell %}
$ python test_selenium.py 
===============================================
<title>株式会社 アセントネットワークス</title>
===============================================
{% endhighlight %}
